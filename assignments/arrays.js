...
// Assuming they don't want us to just use an index
for (let idx = 0; idx < inventory.length; idx++) {
    if (inventory[idx].id === 33) {
        console.log("Car 33 is a "
                    + inventory[idx].car_year + " "
                    + inventory[idx].car_make + " "
                    + inventory[idx].car_model);
    }
}

let car33 = inventory.filter(car => car.id === 33)[0];
...
// entirely unnecessary and bad functional solution
lastCar = inventory.reduce((_, car) => car);
console.log(lastCar.car_make + " " + lastCar.car_model);
...
let carModels = [];
for (let idx = 0; idx < inventory.length; idx++) {
    carModels.push(inventory[idx].car_model);
}
// Asumming they don't want us to use sort
// carModels.sort();
function insertionSort(arr) {
    for (let uidx = 1; uidx < arr.length; uidx++) {
        let uval = arr[uidx];
        let sidx = uidx - 1;
        while (sidx >= 0 && arr[sidx] > uval) {
            arr[sidx+1] = arr[sidx];
            sidx--;
        }
        arr[sidx + 1] = uval;
...
let carYears = [];
for (let idx = 0; idx < inventory.length; idx++) {
    carYears.push(inventory[idx].car_year);
}
...
let oldCars = [];
for (let idx = 0; idx < carYears.length; idx++) {
    if (carYears[idx] < 2000) {
        oldCars.push(carYears[idx]);
    }
}
console.log(oldCars.length); 

console.log(inventory.map(car => car.car_year).filter(year => year < 2000).length);
